...
export async function getInventoryStockDetails(): Promise<ProductWithStockDetails[]> {
    try {
        ...
        const [productsRes] = await db.query(productsQuery) as [ProductQueryResult[], any];
        const [stockDetailsRes] = await db.query(stockDetailsQuery) as [StockDetailQueryResult[], any];
...
export async function getWarehouseStructure() {
    try {
        ...
        const [warehousesRes] = await db.query(warehousesQuery) as [WarehouseRow[], any];
        const [sectionsRes] = await db.query(sectionsQuery) as [SectionRow[], any];
        const [coordinatesRes] = await db.query(coordinatesQuery) as [CoordinateRow[], any];
...
export async function getProductsForSelect() {
    try {
        const [rows] = await db.query('SELECT id, sku, nombre FROM cat_productos WHERE activo = 1 ORDER BY sku') as [RowDataPacket[], any];
